import os  # To help define the path to the database file
...
)  # the path for the application itself.
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + os.path.join(
    basedir, "planets.db"
)
app.config["JWT_SECRET_KEY"] = "super secret"
app.config["MAIL_SERVER"] = "smtp.mailtrap.io"
app.config["MAIL_USERNAME"] = "114d82b2940ba7"
app.config["MAIL_PASSWORD"] = "7a83000483d916"
...
def login():
    ...
    ).first()  # .first() is either going to return something or not.
    if test:
...
# Retrieving a single planet details
...
def add_planet():
    ...
    if test:
        ...
        db.session.commit()
        return jsonify(message="You added a new planet ^_^"), 201
